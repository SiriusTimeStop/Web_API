...
export const getSearch = async  (sfield:any,q:any) =>{
 const query = `SELECT ${sfield} FROM registerusers WHERE ${sfield} LIKE '%${q}%' `;
...
export const getByUserId = async  (id:number) =>{
  ...
  parm=parm.slice(0,-1)
  let query = `INSERT INTO registerusers (${key}) VALUES (${parm})`
  try{
...
export const findByUsername = async (username: string) => {
  const query = 'SELECT * FROM registerusers where username = ?';
...
export const  update= async(user:any,id:any)  =>{  
 ...
 // console.log("updateString ", updateString)
  let query = `UPDATE registerusers SET ${updateString} WHERE ID=${id} RETURNING *;`
  try{
   await db.run_query(query, values)  
    return {"status": 201}
  } catch(error) {
...
export const deleteById = async (id:any) => {
  let query = "Delete FROM registerusers WHERE ID = ?"
  let values = [id]
  try{
    await db.run_query(query, values);  
